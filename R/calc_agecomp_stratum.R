...
calc_agecomp_stratum <- function(racebase_tables = NULL,
  ...
  age_comp <- (alk[size_comp[SPECIES_CODE %in% unique(x = alk$SPECIES_CODE)],
                   on = c("SURVEY_DEFINITION_ID", "SURVEY",
                          "YEAR", "SPECIES_CODE", "SEX", "LENGTH_MM"), allow.cartesian = T])
  ...
  #         all.x = TRUE)
  # age_comp2$AGE[is.na(x = age_comp2$AGE)] <- -9
  # age_comp2$AGE_FRAC[is.na(x = age_comp2$AGE_FRAC)] <- 1
  ...
  # age_comp2$AGEPOP <- age_comp2$AGE_FRAC * age_comp2$POPULATION_COUNT
  # 
  # count_length_age2 <- age_comp2
  # count_length_age2$AGE[count_length_age2$LENGTH_MM < 0] <- -99
  ...
  # mean_length_at_age2 <- data.frame()
  # 
  # for (ispp in sort(unique(alk$SPECIES_CODE))){
  # 
  #   mean_length_at_age2 <-
  #     rbind(mean_length_at_age2,
  #           do.call(
  #             what = rbind,
  #             args = lapply(
  #               X = split(x = subset(x = age_comp2,
  #                                    subset = LENGTH_MM > 0 &
  #                                      SPECIES_CODE == ispp),
  #                         f = with(subset(x = age_comp2,
  #                                         subset = LENGTH_MM > 0 &
  #                                           SPECIES_CODE == ispp),
  #                                  list(SURVEY, YEAR,
  #                                       STRATUM, SEX, AGE))),
  ...
  # }
  # 
  # rownames(x = mean_length_at_age2) <- NULL
  ...
  # age_comp2 <- stats::aggregate(AGEPOP ~ SURVEY + YEAR + STRATUM +
  #                                SPECIES_CODE + SEX + AGE,
  #                              data = age_comp2,
  ...
  # age_comp2 <- merge(x = age_comp2,
  #                   y = subset(x = mean_length_at_age2,
  ...
  # names(x = age_comp2)[names(age_comp2) == "AGEPOP"] <- "POPULATION_COUNT"
  ...
  # age_comp2$AGE[is.na(x = age_comp2$LENGTH_MM_MEAN)] <- -99
