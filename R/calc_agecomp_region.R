...
calc_agecomp_region <- function(racebase_tables,
  ...
  subarea_age_comp <- 
    subarea_age_comp[age_comps_stratum$length_at_age,
                     on = c("SURVEY_DEFINITION_ID", "YEAR ", "STRATUM"),
  ...
  mean_length_at_age[AGE == -99, 
                     c("LENGTH_MM_MEAN", "LENGTH_MM_SD")] <- NA
  
  
  mean_length_at_age <-
    mean_length_at_age[order(SURVEY_DEFINITION_ID, SURVEY, YEAR, 
                               AREA_ID, SPECIES_CODE, SEX, AGE)]
  mean_length_at_age <- mean_length_at_age[, -"SURVEY"]
  
  # subarea_age_comp <- 
  #   mean_length_at_age[subarea_age_comp,
