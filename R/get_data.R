...
get_data <- function(year_set = c(1996, 1999),
  ...
  ## Removing temporary tables if they exist
  for (itable in c("AVAIL_SPP", "CATCH", "CRUISE", "HAUL", "INPUT_SPP", 
    ...
    if (nrow(x = RODBC::sqlQuery(channel = sql_channel,
...
AND YEAR IN", year_vec)
  ...
  RODBC::sqlQuery(channel = sql_channel, query = survey_sql)
...
FROM GAPINDEX_TEMPORARY_SURVEY_QUERY"
  ...
  RODBC::sqlQuery(channel = sql_channel, query = survey_design_sql)
...
AND FLOOR(B.CRUISE/100) IN", year_vec)
  ...
  RODBC::sqlQuery(channel = sql_channel, query = cruise_sql)
...
AND AREA.SURVEY_DEFINITION_ID IN", survey_def_ids_vec,
  ...
  RODBC::sqlQuery(channel = sql_channel, query = stratum_sql)
...
AND AREA.SURVEY_DEFINITION_ID IN", survey_def_ids_vec,
  ...
  RODBC::sqlQuery(channel = sql_channel, query = subarea_sql)
...
ORDER BY DESIGN_YEAR, SURVEY, AREA_ID, STRATUM")
  ...
  RODBC::sqlQuery(channel = sql_channel, query = stratum_groups_sql)
...
AND ABUNDANCE_HAUL IN", gapindex::stitch_entries(abundance_haul), 
    ifelse(test = na_rm_strata == T,
           yes = "\nAND STRATUM IS NOT NULL",
           no = "")
  )
  
  RODBC::sqlQuery(channel = sql_channel, query = haul_sql)
...
ORDER BY SPECIES_CODE"
  ...
  RODBC::sqlQuery(channel = sql_channel, query = avail_spp_sql)
  ...
  if (is.data.frame(x = spp_codes)) {
    ...
    RODBC::sqlSave(channel = sql_channel, dat = spp_codes, 
...
JOIN RACEBASE.SPECIES_CLASSIFICATION USING (SPECIES_CODE)"
    ...
    RODBC::sqlQuery(channel = sql_channel, query = species_sql)
...
# )"
#   RODBC::sqlQuery(channel = sql_channel, query = unavailable_spp_sql)
...
-- Aggregate numbers and weights by GROUP_CODE
...
")
  ...
  RODBC::sqlQuery(channel = sql_channel, query = catch_sql)
...
WITH SIZE_QUERY AS (
  SELECT HAULJOIN, SPECIES_CODE, GROUP_CODE, SEX, LENGTH, FREQUENCY
...
SELECT HAULJOIN, GROUP_CODE AS SPECIES_CODE, SEX, LENGTH, 
SUM(FREQUENCY) AS FREQUENCY

FROM SIZE_QUERY

GROUP BY (HAULJOIN, GROUP_CODE, SEX, LENGTH)

ORDER BY HAULJOIN, SPECIES_CODE, LENGTH, SEX")
    
    RODBC::sqlQuery(channel = sql_channel, query = size_sql) 
...
ORDER BY HAULJOIN, SPECIES_CODE, AGE, LENGTH"
    ...
    RODBC::sqlQuery(channel = sql_channel, query = specimen_sql) 
