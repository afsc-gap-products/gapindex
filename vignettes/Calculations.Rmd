...
This is represented in the `get_biomass_stratum()` function as follows:
```{r eval=FALSE}
x2 <- x %>%
    group_by(year, stratum) %>%
    dplyr::summarize(
...
x <- data.frame(symbol = c("A", "B", "c", "CPUE", "D", "F","L","P","S" ), 
                ...
                units = c("$km^2$","$kg$","$scalar$","$kg/ha$","$km$",
                          ...
                          "$n per hectare$"))
kableExtra::kbl(x, escape = FALSE,caption = "Variable names used in these calculations") %>%
  kable_styling()
```
